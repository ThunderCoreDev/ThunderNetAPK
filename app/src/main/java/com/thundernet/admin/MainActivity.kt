...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            try {
                ...
                // Inicializar splashLayout de forma segura
    ...
    private fun setupSplashAnimations() {
        try {
            ...
            // ANIMACIÓN 2: Fade in para el logo (solo si existe)
            wowLogo?.alpha = 0f
            wowLogo?.animate()
                ?.alpha(1f)
                ?.setDuration(1000)
            ...
            // ANIMACIÓN 3: Texto "Conectando..." con puntos animados (solo si existe)
            loadingText?.let {
                animateLoadingText(it)
            }
            
            // ANIMACIÓN 4: Parpadeo sutil del logo (solo si existe)
            wowLogo?.let { logo ->
    ...
    private fun stopSplashAnimations() {
        try {
            // Detener animación del círculo (de forma segura)
            val loadingCircle = findViewById<ImageView?>(R.id.loadingCircle)
            loadingCircle?.clearAnimation()
            
            // Detener animación del logo (de forma segura)
            val wowLogo = findViewById<ImageView?>(R.id.wowLogo)
    ...
    private fun loadWebView() {
        ...
        try {
            ...
            // Inicializar WebView de forma segura
            val webView = findViewById<WebView?>(R.id.webView)
            if (webView == null) {
                showErrorCrash("WebView no encontrado en el layout")
                return
            }
            this.webView = webView
            
            // Inicializar otras vistas de forma segura con operador ?
            ...
            // Configurar botón de menú (solo si existe)
            menuButton?.setOnClickListener {
                showOptionsMenu(it)
            }
            
            // Configurar botón de reintentar (solo si existe)
